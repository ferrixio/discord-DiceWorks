...
class diceBlock:
    ...
    def buildDice(v:str):
        ...
        for k in plus_item:
            ...
            for z in minus_item:
                ...
                if first:
                    first = False
                    R,dado = diceBlock.check_roll(z,True)
                else:
                    R,dado = diceBlock.check_roll(z,False)
    ...
    def check_roll(x:str, B:bool):
        ...
        if "d" in x:
            ...
            ZZ = diceBlock.roll(quantity,die_amplitude,B)
    ...
    def standard_roll(L:list):
        ...
        for item in G:
            tempResults, _ = diceBlock.buildDice(item)
        ...
        finalResults = diceBlock.erase_parethesis(finalResults,'')
    ...
    def van_svg(L:list, term:str, name:str):
        ...
        for item in G:
            tempResults, die = diceBlock.buildDice(item)
            if isinstance(tempResults, str):
    ...
    def forall(L:list):
        ...
        rolledResults = diceBlock.erase_parethesis(rolledResults,'')
        ...
        for item in G:
            ...
            for i in tempResults:
                if isinstance(i, list) and len(i)==1:
                    shian = diceBlock.explode(i[0],die[0])
